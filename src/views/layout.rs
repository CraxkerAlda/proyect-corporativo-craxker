pub fn layout_maestro(titulo: &str, contenido: String) -> String {
    ...
    <head>
        ...
        <title href="/dashboard">{titulo} | Craxker Design Hub</title>
        ...
        <style>
            ...
            .top-nav {{ background: var(--primary-blue); color: white; padding: 0.8rem 2rem; display: flex; justify-content: space-between; align-items: center; border-bottom: 4px solid var(--accent-green); position: sticky; top: 0; z-index: 1000; }}
            ...
            .user-avatar {{ width: 32px; height: 32px; border-radius: 50%; object-fit: cover; border: 2px solid var(--accent-green); }}
        ...
        <script>
            async function aplicarPermisosAcciones(nombreModulo, accionRequerida = null) {{
                ...
                try {{
                    ...
                    // Si se requiere una acción específica, verificar ese bit puntualmente
                    if (accionRequerida !== null && !p[accionRequerida]) {{
                        window.location.href = '/error-403';
                        return;
                    }}

                    // Ocultar botones de acción según bits de permiso
            ...
            document.addEventListener('DOMContentLoaded', async () => {{
                ...
                document.getElementById('nav-user-img').src = localStorage.getItem('user_img') || '/uploads/default.png';
                ...
                try {{
                    ...
                    if (res.ok) {{
                        ...
                        for (const [nombrePadre, modulos] of Object.entries(grupos)) {{
                            ...
                            if (nombrePadre === 'Principal 1') icon = 'th-large';
